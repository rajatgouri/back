...
const { MASTER_KEY, PRIVATE_KEY, PUBLIC_KEY, PAYDUNYA_TOKEN ,REDIRECT_URL ,CANCEL_URL } = process.env;
...
exports.processPayment = (req, res) => {
    User.findOne({ _id: req.body.ride.user  }, (err, user) => {
        ...
        invoice.addItem('Carpooling', req.body.ride.seats,500);
        invoice.totalAmount = +req.body.ride.total + 500*(+req.body.ride.seats);
        invoice.create()
            .then(function (){
                const payment = {
                    ...
                    token: invoice.token,
                    total: +req.body.ride.total + 500*(+req.body.ride.seats),
                ...
                Payment.create(payment, function (err) {
                    ...
                    return res.status(200).send({url:invoice.url})
                  });
            })
            .catch(function (e) {
                console.log(e);
