...
exports.emailOtpSend = (req, res) => {
  ...
  User.findOne({ email: req.query.email }, (err, user) => {
    ...
    .catch(err => {
      return res.status(400).send({msg: "Otp not sent please try again"})
    });
  })
};
...
exports.verifyEmail = (req, res) => {
  ...
  User.findOne({ email: req.body.email }, (err, user) => {
    ...
    if (user.emailOtp == req.body.otp) {
      ...
      .catch(err => {
        return res.status(400).send({msg: "Some error occured , please try again"})
      });
    } else {
      return res.status(400).send({msg: "Wrong OTP , Please enter correct OTP!"})
    }
  })
};
...
exports.forgotOtpCheck = async (req, res) => {
  ...
  User.findOne({ email: req.body.email }, (err, user) => {
    ...
    if (user.emailOtp == req.body.otp) {
      ...
      user.save()
      .then(result=>{
        return res.status(200).send({msg: "Email verified successfully",token: createToken(result)})
      })
      .catch(err => {
        return res.status(200).send({msg: "Some error occured , please try again",})
      });
    } else {
      return res.status(400).send({msg: "Wrong OTP , Please enter correct OTP!"})
    }
