...
exports.addRide = (req, res) => {
  ...
  User.findOne({ _id: req.body.ride.driver  }, (err, user) => {
    ...
    Ride.create(ride, function (err,ride) {
      ...
      return res.status(200).send({msg: "Ride Added successfully", data: ride})
    });
  })
};
...
exports.upcomingRides = (req, res) => {
  Ride.find({}, (err, rides) => {
    ...
    rides = rides.sort(function(b,a){
      return  new Date(b.date) -  new Date(a.date);
    });
    rides = rides.slice(0,3);
    rides = rides.length > 0 ? rides : null;
    return res.status(200).send({upcoming: rides})
  })
};
...
exports.getMyRides = (req, res) => {
  ...
  Ride.find({ driver: req.query.id}, (err, rides) => {
    ...
    User.findById(req.query.id , (err, user) => {
      ...
      isIdSubmitted = user.isIdSubmitted;
      return res.status(200).send({myRides: rides , isIdSubmitted: isIdSubmitted})
    })
  })
};
...
exports.getClientRides = (req, res) => {
  ...
  Ride.find({ "bookings.client": req.query.id}, (err, rides) => {
    ...
    User.findById(req.query.id , (err, user) => {
      ...
      isIdSubmitted = user.isIdSubmitted;
      return res.status(200).send({clientRides: rides , isIdSubmitted: isIdSubmitted})
    })
  })
};
...
exports.getRideById = (req, res) => {
  ...
  Ride.findById(req.query.id, (err, ride) => {
    ...
    User.findById(ride.driver, (err2,user) => {
      ...
      return res.status(200).send({ride: ride , driver: user})
    })
  })
};
